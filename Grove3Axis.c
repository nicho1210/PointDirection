...
void Grove3Axis_setup(int fp) {
    ...
    printf("Constructing new HMC5883L\n");

    printf("Setting scale to +/- 1.3 Ga\n");
    error = setScale(fp, 1.3); // Set the scale of the compass.
    if (error != 0) { // If there is an error, print it out.
        printf("Error = %s\n", getErrorText(error));
    }

    printf("Setting measurement mode to continous.\n");
    setMeasurementMode(fp, HMC5883L_CONTINOUS); // Set the measurement mode to Continuous
...
void compassCalibrate(int fp) {
    ...
    MagnetometerScaled value = {0, 0, 0};
    int xcount = 0;
    int ycount = 0;
    int zcount = 0;

    printf("calibrate the compass\n");
    // calculate x, y and z offset
    // printf("please rotate the compass\n");
    ...
    printf("Max: X=%f Y=%f Z=%f\n", valueMax.XAxis, valueMax.YAxis, valueMax.ZAxis);
    printf("Min: X=%f Y=%f Z=%f\n", valueMin.XAxis, valueMin.YAxis, valueMin.ZAxis);
...
void Grove3Axis_loop(int fp, float *xAxis) {
    ...
    //MagnetometerRaw raw = readRawAxis(fp);
    // Retrived the scaled values from the compass (scaled to the configured scale).
    MagnetometerScaled scaled = readScaledAxis(fp);
...
//    float zxHeading = atan2(scaled.ZAxis, scaled.XAxis);
    ...
    //pryd float declinationAngle = -0.0457;
    //pryd >
    /* Taipei -4`47 = -0.0456752665 radians 
    //float declinationAngle = (-4.0 + -(47.0 / 60.0)) / (180 / M_PI) = -4.7833 / 57.2975 = -0.0835
    */
    //pryd <
    float declinationAngle = -0.0835;       //pryd
